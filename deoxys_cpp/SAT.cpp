...
void SATsolverLinear(vector<vector<int>> linearKeys, uint32_t **linearValues, uint32_t *linearValuesLength, int TK_NUM)
{
    ...
    {
    	vector<uint32_t> cells;
    	for (uint32_t j = 0; j < linearKeys[i].size(); j++) cells.push_back((linearKeys[i][j] >> (2*SIZE)) & 0xf); // positions of the keys
    	uint64_t freeVarIncrement = freeVarPoint;
    	uint64_t incrementCounter = 0;
    	for (uint64_t j = 0; j < (1ULL<<(SIZE*linearValuesLength[i])); j++)
    	{
    		if (linearValues[i][j] > 0)
    		{
    			...
    			Tseitin(clause,solver,cells,j,TK_NUM,freeVarIncrement,noOfClauses); // j is the key val (TK1 key1)(TK2 key1)(TK1 key2)(TK2 key2)
    			freeVarIncrement++;
    		}
    	}
    	// combine to add the final clause in (final step of the Tsetin transformation)
