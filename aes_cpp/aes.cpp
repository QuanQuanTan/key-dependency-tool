#include <iostream>
#include <iomanip>
#include "aes.h"

using namespace std;

uint8_t RCON[11] = {0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36};
uint8_t Sbox[256] = 
{};

uint8_t invSbox[256] = {
};

uint8_t getSbox(uint8_t val) {return Sbox[val];}
uint8_t getInvSbox(uint8_t val) {return invSbox[val];}
uint8_t getConstants(uint8_t val) {return RCON[val];}

uint8_t mul2(uint8_t val)
{
	if ((val >> 7) == 1)
		return ((val << 1) ^ 0b00011011);
	else
		return (val << 1);
}
uint8_t mul3(uint8_t val)
{
	return mul2(val) ^ val;
}

void keyScheduleRoundFunction(uint8_t key_state[4][8], uint8_t expandedKey[15][4][4],uint32_t keySize = 128)
{
	uint8_t tmp[4][60];
	int required;
	int nextCol = 0;
	int N = keySize / 32;

	if (keySize == 128) required = 11 * 4;
	if (keySize == 192) required = 13 * 4;
	if (keySize == 256) required = 15 * 4;
	while (nextCol < required)
	{
		if (nextCol < N)
		{
			for (int r = 0; r < 4; r++) tmp[r][nextCol] = key_state[r][nextCol];
		}
		else if ((nextCol >= N) && ((nextCol % N) == 0))
		{
		    tmp[0][nextCol] = tmp[0][nextCol-N] ^ Sbox[tmp[1][nextCol-1]] ^ RCON[nextCol/N];
			for (int r = 1; r < 4; r++) tmp[r][nextCol] = tmp[r][nextCol-N] ^ Sbox[tmp[(r+1)%4][nextCol-1]];
		}
		else if ((nextCol > N) && (N > 6) && ((nextCol % N) == 4))
		{
			for (int r = 0; r < 4; r++) tmp[r][nextCol] = Sbox[tmp[r][nextCol-1]] ^ tmp[r][nextCol-N];
		}
		else
		{
			for (int r = 0; r < 4; r++) tmp[r][nextCol] = tmp[r][nextCol-1] ^ tmp[r][nextCol-N];
		}
		nextCol++;
	}
	for (int n = 0; n < 15; n++)
	{
		for (int r = 0; r < 4; r++)
		{
			for (int c = 0; c < 4; c++) expandedKey[n][r][c] = tmp[r][4*n+c];
		}
	}
}


void Substitution(uint8_t state[4][4])
{
	for (int r = 0; r < 4; r++)
	{
		for (int c = 0; c < 4; c++) state[r][c] = Sbox[state[r][c]];
	}
}

void invSubstitution(uint8_t state[4][4])
{
	for (int r = 0; r < 4; r++)
	{
		for (int c = 0; c < 4; c++) state[r][c] = invSbox[state[r][c]];
	}
}

void SR(uint8_t state[4][4])
{
	uint8_t tmp[4][4];
	for (int r = 0; r < 4; r++)
	{
		for (int c = 0; c < 4; c++) tmp[r][c] = state[r][(c+r)%4];
	}
	for (int r = 0; r < 4; r++)
	{
		for (int c = 0; c < 4; c++) state[r][c] = tmp[r][c];
	}
}

void invSR(uint8_t state[4][4])
{
	uint8_t tmp[4][4];
	for (int r = 0; r < 4; r++)
	{
		for (int c = 0; c < 4; c++) tmp[r][c] = state[r][(4+c-r)%4];
	}
	for (int r = 0; r < 4; r++)
	{
		for (int c = 0; c < 4; c++) state[r][c] = tmp[r][c];
	}
}


void MC(uint8_t state[4][4])
{
	uint8_t tmp[4][4];
	for (int c = 0; c < 4; c++)
	{
		tmp[0][c] = mul2(state[0][c]) ^ mul3(state[1][c]) ^ state[2][c] ^ state[3][c];
		tmp[1][c] = state[0][c] ^ mul2(state[1][c]) ^ mul3(state[2][c]) ^ state[3][c];
		tmp[2][c] = state[0][c] ^ state[1][c] ^ mul2(state[2][c]) ^ mul3(state[3][c]);
		tmp[3][c] = mul3(state[0][c]) ^ state[1][c] ^ state[2][c] ^ mul2(state[3][c]);
	}
	for (int r = 0; r < 4; r++)
	{
		for (int c = 0; c < 4; c++) state[r][c] = tmp[r][c];
	}
}

void invMC(uint8_t state[4][4])
{
	MC(state);
	MC(state);
	MC(state);
}

void AddRoundKey(uint8_t state[4][4], uint8_t key[4][4])
{
	for (int r = 0; r < 4; r++)
	{
		for (int c = 0; c < 4; c++)
		{
			state[r][c] ^= key[r][c];
		}
	}
}

void RoundFunction(uint8_t state[4][4], uint8_t key[4][4])
{
	Substitution(state);
	SR(state);
	MC(state);
	AddRoundKey(state,key);
}

void InvRoundFunction(uint8_t state[4][4], uint8_t key[4][4])
{
	AddRoundKey(state,key);
	invMC(state);
	invSR(state);
	invSubstitution(state);

}

void aes_encrypt(uint8_t state[4][4], uint8_t key[4][8], int nr, int keySize)
{
	uint8_t expandedKey[15][4][4];
	keyScheduleRoundFunction(key,expandedKey,keySize);
	AddRoundKey(state,expandedKey[0]);
	for (int r = 1; r < nr; r++) 
	{ 
		RoundFunction(state,expandedKey[r]);
	}
	Substitution(state);
	SR(state);
	AddRoundKey(state,expandedKey[nr]);
}

void aes_decrypt(uint8_t state[4][4], uint8_t key[4][8], int nr, int keySize)
{
	uint8_t expandedKey[15][4][4];
	keyScheduleRoundFunction(key,expandedKey,keySize);
	AddRoundKey(state,expandedKey[nr]);
	invSR(state);
	invSubstitution(state);
	for (int r = nr-1; r >= 1; r--) InvRoundFunction(state,expandedKey[r]);
	AddRoundKey(state,expandedKey[0]);
}


void transpose(uint8_t a[4][4])
{
	uint8_t tmp[4][4];
	for (int r = 0; r < 4; r++)
	{
		for (int c = 0; c < 4; c++) tmp[c][r] = a[r][c];
	}
	for (int r = 0; r < 4; r++)
	{
		for (int c = 0; c < 4; c++) a[r][c] = tmp[r][c];
	}
}

void computeDDT(uint32_t DDT[256][256])
{
	for (int i = 0; i < 256; i++)
	{
		for (int j = 0; j < 256; j++) DDT[i][j] = 0;
	}
	for (int i = 0; i < 256; i++)
	{
		for (int j = 0; j < 256; j++) DDT[i^j][Sbox[i]^Sbox[j]]++;
	}
}

void computeXDDT(uint8_t x, uint8_t y, uint32_t xddt[256])
{
	for (int i = 0; i < 256; i++) xddt[i] = 0;
	for (int i = 0; i < 256; i++)
	{
		for (int j = 0; j < 256; j++) 
		{
			if (((i^j) == x) && ((Sbox[i]^Sbox[j]) == y)) xddt[i]++;
		}
	}

}

void computeYDDT(uint8_t x, uint8_t y, uint32_t yddt[256])
{
	for (int i = 0; i < 256; i++) yddt[i] = 0;
	for (int i = 0; i < 256; i++)
	{
		for (int j = 0; j < 256; j++) 
		{
			if (((i^j) == x) && ((Sbox[i]^Sbox[j]) == y)) yddt[Sbox[i]]++;
		}
	}
}


// change this
void test_vectors()
{
	uint8_t state11[4][4] = {{0x00,0x44,0x88,0xcc},{0x11,0x55,0x99,0xdd},{0x22,0x66,0xaa,0xee},{0x33,0x77,0xbb,0xff}};
	uint8_t state12[4][4] = {{0x00,0x44,0x88,0xcc},{0x11,0x55,0x99,0xdd},{0x22,0x66,0xaa,0xee},{0x33,0x77,0xbb,0xff}};
	uint8_t key11[4][8] = {
	{0x00,0x04,0x08,0x0c},{0x01,0x05,0x09,0x0d},{0x02,0x06,0x0a,0x0e},{0x03,0x07,0x0b,0x0f}};
	uint8_t key12[4][8] = {
	{0x00,0x04,0x08,0x0c},{0x01,0x05,0x09,0x0d},{0x02,0x06,0x0a,0x0e},{0x03,0x07,0x0b,0x0f}};
	uint8_t expected1[4][4] = {{0x69,0x6a,0xd8,0x70},{0xc4,0x7b,0xcd,0xb4},{0xe0,0x04,0xb7,0xc5},{0xd8,0x30,0x80,0x5a}};
	aes_encrypt(state11,key11,10,128);
	for (int r = 0; r < 4; r++)
	{
		for (int c = 0; c < 4; c++) if (state11[r][c] != expected1[r][c]) {cout << "Test vector 1 (enc) failed!" << endl; exit(0);}
	}
	aes_decrypt(state11,key12,10,128);
	for (int r = 0; r < 4; r++)
	{
		for (int c = 0; c < 4; c++) if (state11[r][c] != state12[r][c]) {cout << "Test vector 1 (dec) failed!" << endl; exit(0);}
	}
	
	// test vector 2
	uint8_t state21[4][4] = {{0x00,0x44,0x88,0xcc},{0x11,0x55,0x99,0xdd},{0x22,0x66,0xaa,0xee},{0x33,0x77,0xbb,0xff}};
	uint8_t state22[4][4] = {{0x00,0x44,0x88,0xcc},{0x11,0x55,0x99,0xdd},{0x22,0x66,0xaa,0xee},{0x33,0x77,0xbb,0xff}};
	uint8_t key21[4][8] = {
	{0x00,0x04,0x08,0x0c,0x10,0x14},{0x01,0x05,0x09,0x0d,0x11,0x15},{0x02,0x06,0x0a,0x0e,0x12,0x16},{0x03,0x07,0x0b,0x0f,0x13,0x17}};
	uint8_t key22[4][8] = {
	{0x00,0x04,0x08,0x0c,0x10,0x14},{0x01,0x05,0x09,0x0d,0x11,0x15},{0x02,0x06,0x0a,0x0e,0x12,0x16},{0x03,0x07,0x0b,0x0f,0x13,0x17}};
	uint8_t expected2[4][4] = {{0xdd,0x86,0x6e,0xec},{0xa9,0x4c,0xaf,0x0d},{0x7c,0xdf,0x70,0x71},{0xa4,0xe0,0xa0,0x91}};
	aes_encrypt(state21,key21,12,192);
	for (int r = 0; r < 4; r++)
	{
		for (int c = 0; c < 4; c++) if (state21[r][c] != expected2[r][c]) {cout << "Test vector 2 (enc) failed!" << endl; exit(0);}
	}
	aes_decrypt(state21,key22,12,192);
	for (int r = 0; r < 4; r++)
	{
		for (int c = 0; c < 4; c++) if (state21[r][c] != state22[r][c]) {cout << "Test vector 2 (dec) failed!" << endl; exit(0);}
	}
	
	// test vector 3
	uint8_t state31[4][4] = {{0x00,0x44,0x88,0xcc},{0x11,0x55,0x99,0xdd},{0x22,0x66,0xaa,0xee},{0x33,0x77,0xbb,0xff}};
	uint8_t state32[4][4] = {{0x00,0x44,0x88,0xcc},{0x11,0x55,0x99,0xdd},{0x22,0x66,0xaa,0xee},{0x33,0x77,0xbb,0xff}};
	uint8_t key31[4][8] = {
	{0x00,0x04,0x08,0x0c,0x10,0x14,0x18,0x1c},{0x01,0x05,0x09,0x0d,0x11,0x15,0x19,0x1d},
	{0x02,0x06,0x0a,0x0e,0x12,0x16,0x1a,0x1e},{0x03,0x07,0x0b,0x0f,0x13,0x17,0x1b,0x1f}};
	uint8_t key32[4][8] = {
	{0x00,0x04,0x08,0x0c,0x10,0x14,0x18,0x1c},{0x01,0x05,0x09,0x0d,0x11,0x15,0x19,0x1d},
	{0x02,0x06,0x0a,0x0e,0x12,0x16,0x1a,0x1e},{0x03,0x07,0x0b,0x0f,0x13,0x17,0x1b,0x1f}};
	uint8_t expected3[4][4] = {{0x8e,0x51,0xea,0x4b},{0xa2,0x67,0xfc,0x49},{0xb7,0x45,0x49,0x60},{0xca,0xbf,0x90,0x89}};
	aes_encrypt(state31,key31,14,256);
	for (int r = 0; r < 4; r++)
	{
		for (int c = 0; c < 4; c++) if (state31[r][c] != expected3[r][c]) {cout << "Test vector 3 (enc) failed!" << endl; exit(0);}
	}
	aes_decrypt(state31,key32,14,256);
	for (int r = 0; r < 4; r++)
	{
		for (int c = 0; c < 4; c++) if (state31[r][c] != state32[r][c]) {cout << "Test vector 3 (dec) failed!" << endl; exit(0);}
	}


	
}

// int main()
// {
// 	test_vectors();

// 	return 0;
// }