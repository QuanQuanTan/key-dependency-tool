...
void solve(uint8_t alpha[30][16], vector<vector<vector<int>>> xPositions, vector<vector<int>> xParity, vector<vector<vector<int>>> yPositions, vector<vector<int>> yParity, int nr, int size)
{
	...
	// the indices 127 - 0 are reserved for the master key/ round key
	...
	2. (Reason) 4n+1 involves inactive Sboxes from round 1,5,9,... so the round keys involved from these rounds are the same
	...
	5. Once we have retrieved them, we use a sat solver to find out if they are compatible
	...
	*/
	...
	// first change: put all the clauses into a vector first, before putting them into the solver
	
	#if SIZE == 4
	for (int batch = 0; batch < 4; batch++){
		cout << "Performing batch number: " << batch << endl;
		// temp storage to sieve out the shortlisted batch
		vector<vector<vector<int>>> xPositions_temp, yPositions_temp;
		vector<vector<int>> xParity_temp, yParity_temp;
		int tmpIndex;
